...
generate_ca_key_cert () {
	...
	openssl req -new -x509 -days 36500 -key $1/ca.key -out $1/ca.pem -subj /CN=dcae-hv-ves-ca
}

generate_server_key_csr () {
	openssl genrsa -out $1/server.key 2048
	openssl req -new -key $1/server.key -out $1/server.csr -subj /CN=dcae-hv-ves-collector
}

generate_client_key_csr () {
	openssl genrsa -out $1/client.key 2048
	openssl req -new -key $1/client.key -out $1/client.csr -subj /CN=dcae-hv-ves-client
}
...
generate_ca_key_cert "$DIR/$ETESCRIPTDIR"
generate_server_key_csr "$DIR/$ETESCRIPTDIR"
generate_client_key_csr "$DIR/$ETESCRIPTDIR"
sign_server_and_client_cert "$DIR/$ETESCRIPTDIR"
create_pkcs12_ca_and_server "$DIR/$ETESCRIPTDIR"
copy_server_certs_to_hvves "$DIR/$ETESCRIPTDIR" "$NAMESPACE" "$HVVESPOD" "/tmp"
copy_client_certs_to_robot "$DIR/$ETESCRIPTDIR" "$NAMESPACE" "$POD" "/tmp"
cleanup "$DIR/$ETESCRIPTDIR"
